...
- รองรับ Thai optimization ผ่าน Thonburian model + Google STT fallback
...
- Cloud storage backend (GCS/MinIO/S3) — เตรียม interface ไว้แล้ว แต่ยังไม่ implement
- Unit tests — ยังไม่มี
...
- `_transcribe_with_whisper()` → Full transcription + language detection
...
- Features: Adaptive beam sizes per model, VAD filtering, error recovery
...
- Models: `DEFAULT_MODEL_SIZE`, `THONBURIAN_MODEL`, standard Whisper sizes
- `ADAPTIVE_BEAM_SIZES`: per-model detection/transcription beam sizes
- VAD: threshold, min speech/silence duration
